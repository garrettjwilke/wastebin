...
BASE_URL="http://homeless.wtf:8086"
...
optionExpires=0 # amount of time in seconds before paste will be deleted
...
optionRaw=false # shows the raw file rather than the rendered html
...
cat <<EOF
                    -----------------------------
                      homeless.wtf paste thingy
                    -----------------------------

pipe any text into this thingy and it will paste to homeless.wtf.


    hmls-paste.sh [ FLAG (OPTIONS) ]
...
send_json() {
  ...
  if ! [ -t 0 ]; then
    ...
    if [ "$optionRaw" == true ]; then
      parsed_url=$(echo "$json_response" | jq -r ". | \"${BASE_URL}\(.path)?fmt=raw\"")
      if [ $? -ne 0 ]; then
